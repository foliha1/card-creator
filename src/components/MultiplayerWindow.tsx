...
const MultiplayerWindow: React.FC<MultiplayerWindowProps> = ({ initialRoomCode, introStatus = "none" }) => {
  ...
  const shellStyle: React.CSSProperties = {
    ...
    padding: mobile ? 0 : 8,
    paddingTop: mobile ? SITE_HEADER_OFFSET : `calc(20px + ${SITE_HEADER_OFFSET})`,
    paddingBottom: mobile ? 0 : "calc(8px + env(safe-area-inset-bottom))",
    paddingLeft: mobile ? 0 : "calc(8px + env(safe-area-inset-left))",
    paddingRight: mobile ? 0 : "calc(8px + env(safe-area-inset-right))",
  ...
  if (view.kind === "idle") {
    ...
    const idleCard = (
      <div style={{
        ...
        maxWidth: mobile ? "none" : 390,
        background: COLORS.surface,
        border: mobile ? "none" : "2px solid " + COLORS.ink,
        borderRadius: mobile ? 0 : 4,
        padding: 16,
        display: "flex",
        flexDirection: "column",
